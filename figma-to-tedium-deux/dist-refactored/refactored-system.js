...
function computeSizingStyles(node, parentNode) {
    ...
    if (node.layoutPositioning !== 'ABSOLUTE') {
        ...
        // BUT exclude frames that should animate (frames with explicit coordinates)
        // Children of ANY auto-layout frame should get 0px positioning
        var isChildOfAutoLayoutFrame = parentNode &&
            parentNode.type === 'FRAME' &&
            parentNode.layoutMode &&
            (node.type !== 'FRAME' || !hasExplicitCoordinates); // Apply to all children of auto-layout frames, but not frames with explicit coordinates
